...
try:
    ...
    st.error(f"❌ Fehler beim Initialisieren des DataManagers: {e}")
    st.stop()
...
try:
    ...
    st.error(f"❌ Fehler beim Erstellen des LoginManagers: {e}")
    st.stop()

# Falls der Benutzer nicht eingeloggt ist, stoppe den weiteren Code
if "authentication_status" not in st.session_state or not st.session_state["authentication_status"]:
    st.warning("⚠️ Sie sind nicht eingeloggt. Bitte melden Sie sich an.")
    st.stop()
...
st.write("""
Willkommen zum Blutzucker-Tracker! Diese App unterstützt Sie dabei, Ihre Blutzuckerwerte einfach zu erfassen, zu speichern und zu analysieren.  
So behalten Sie Ihre Werte stets im Blick und können langfristige Trends erkennen.
""")

# 👤 Benutzerinfo
username = st.session_state.get("username", "Unbekannter Benutzer")  # Falls `username` nicht existiert, wird ein Standardwert gesetzt
